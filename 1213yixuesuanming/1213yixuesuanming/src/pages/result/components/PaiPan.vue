<template>
	...
	<view class="paipan-container">
		...
		<view class="paipan-table">
			...
			<!-- 支神行 -->
			<view class="table-row zhishen-row">
				<view class="row-label">支神</view>
				<view class="row-item" v-for="(item, index) in tableData" :key="'zs'+index">
					<view class="zhishen-list">
						<text v-for="(zs, zsIdx) in parseZhiShen(item.zhi_shi_shen)" :key="zsIdx"
							class="zhishen-text" :class="getWuxingColorClass(item.icon_bom)">{{ zs }}</text>
					</view>
				</view>
			</view>
			
			<!-- 纳音行 -->
			<view class="table-row nayin-row">
				<view class="row-label">纳音</view>
				<view class="row-item" v-for="(item, index) in tableData" :key="'ny'+index">
					<text class="nayin-text" :class="getNayinColorClass(item.nayin)">{{ item.nayin || '-' }}</text>
				</view>
			</view>
			
			<!-- 空亡行 -->
			<view class="table-row">
				<view class="row-label">空亡</view>
				<view class="row-item" v-for="(item, index) in tableData" :key="'kw'+index">
					<text class="normal-text">{{ item.kongwang || '-' }}</text>
				</view>
			</view>
			
			<!-- 地势行 -->
			<view class="table-row">
				<view class="row-label">地势</view>
				<view class="row-item" v-for="(item, index) in tableData" :key="'ds'+index">
					<text class="normal-text">{{ item.dishi || '-' }}</text>
				</view>
			</view>
			
			<!-- 自坐行 -->
			<view class="table-row">
				<view class="row-label">自坐</view>
				<view class="row-item" v-for="(item, index) in tableData" :key="'zz'+index">
					<text class="normal-text">{{ item.zizuo || '-' }}</text>
				</view>
			</view>
			
			<!-- 神煞行 -->
			<view class="table-row shensha-row">
				<view class="row-label">神煞</view>
				<view class="row-item shensha-item" v-for="(item, index) in tableData" :key="'ss'+index">
					<view class="shensha-list">
						<text v-for="(ss, ssIdx) in parseShenSha(item.shen_sha)" :key="ssIdx" class="shensha-text">{{ ss }}</text>
					</view>
				</view>
			</view>
		</view>
		...
		<!-- 天干地支关系 -->
		<view class="relation-section" v-if="tianGanChong || diZhiChong">
			<view class="relation-row" v-if="tianGanChong">
				<text class="relation-label">天干</text>
				<text class="relation-value">{{ tianGanChong }}</text>
			</view>
			<view class="relation-row" v-if="diZhiChong">
				<text class="relation-label">地支</text>
				<text class="relation-value">{{ diZhiChong }}</text>
			</view>
		</view>
