...
class SiZhu extends Api
{
    ...
    public function getSiZhuRes($record_id)
    {
        ...
        $zao = [
            [
                ...
                'gan_shi_shen' => $year_gan_res['shen_name'] ?? ''  // 年干十神
            ],
            [
                ...
                'gan_shi_shen' => $month_gan_res['shen_name'] ?? ''  // 月干十神
            ],
            [
            ...
            [
                ...
                'gan_shi_shen' => $time_gan_res['shen_name'] ?? ''  // 时干十神
            ]
    ...
    private function getFangWeiData($record_id)
    {
        ...
        $day_gan = '';
        foreach ($record_shen_list as $item) {
            if ($item['shen_in'] == 2) { // shen_in=2 为日干
                $day_gan = $item['gan_zhi_name'];
                break;
            }
        }
        ...
        // 回退逻辑：当原局无某十神时，根据日干反向查询对应天干的方位
        if (!empty($day_gan)) {
            // 事业位回退：原局无正官时，查找日干的正官对应天干
            if (empty($fang_wei['shiyeWei'])) {
                $zheng_guan_gan = Db::name('si_zhu_shi_shen')
                    ->where('ri_gan_name', $day_gan)
                    ->where('shi_shen_name', '正官')
                    ->value('gan_name');
                if ($zheng_guan_gan) {
                    $fw_res = Db::name('gan_zhi_fang_wei')->where('gan_zhi', $zheng_guan_gan)->find();
                    $fang_wei['shiyeWei'] = $fw_res ? $fw_res['fang_wei'] : '';
                    $fang_wei['shiyeDesc'] = '根据日干推算的正官方位,利于事业发展';
                    \think\Log::info("[getFangWeiData] 原局无正官,触发回退逻辑,日干:{$day_gan},正官天干:{$zheng_guan_gan}");
                }
            }

            // 正财位回退：原局无正财时，查找日干的正财对应天干
            if (empty($fang_wei['zhengcaiWei'])) {
                $zheng_cai_gan = Db::name('si_zhu_shi_shen')
                    ->where('ri_gan_name', $day_gan)
                    ->where('shi_shen_name', '正财')
                    ->value('gan_name');
                if ($zheng_cai_gan) {
                    $fw_res = Db::name('gan_zhi_fang_wei')->where('gan_zhi', $zheng_cai_gan)->find();
                    $fang_wei['zhengcaiWei'] = $fw_res ? $fw_res['fang_wei'] : '';
                }
            }

            // 偏财位回退：原局无偏财时，查找日干的偏财对应天干
            if (empty($fang_wei['piancaiWei'])) {
                $pian_cai_gan = Db::name('si_zhu_shi_shen')
                    ->where('ri_gan_name', $day_gan)
                    ->where('shi_shen_name', '偏财')
                    ->value('gan_name');
                if ($pian_cai_gan) {
                    $fw_res = Db::name('gan_zhi_fang_wei')->where('gan_zhi', $pian_cai_gan)->find();
                    $fang_wei['piancaiWei'] = $fw_res ? $fw_res['fang_wei'] : '';
                }
            }

            // 贵人位回退：原局无正印时，查找日干的正印对应天干
            if (empty($fang_wei['guirenWei'])) {
                $zheng_yin_gan = Db::name('si_zhu_shi_shen')
                    ->where('ri_gan_name', $day_gan)
                    ->where('shi_shen_name', '正印')
                    ->value('gan_name');
                if ($zheng_yin_gan) {
                    $fw_res = Db::name('gan_zhi_fang_wei')->where('gan_zhi', $zheng_yin_gan)->find();
                    $fang_wei['guirenWei'] = $fw_res ? $fw_res['fang_wei'] : '';
                    $fang_wei['guirenDesc'] = '根据日干推算的正印方位,利于遇贵人相助';
                }
            }
        }
