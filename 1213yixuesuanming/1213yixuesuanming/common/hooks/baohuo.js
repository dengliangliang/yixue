...
/**
 * 申请权限
 * @param {Object} title 权限未开启提示的消息
 */
export function checkPermission(title) {
	return new Promise((resolve) => {
		plus.android.requestPermissions(
			//需要的权限
			["android.permission.WAKE_LOCK"],
			function(resultObj) {
				console.log('授权数组resultObj===', resultObj); //授权的权限，数组
				if (resultObj.granted.length === 0) {
					console.log('====', resultObj.granted.indexOf('android.permission.WAKE_LOCK'));
					plus.android.requestPermissions(['android.permission.WAKE_LOCK'])
				}
				resolve()
			}
		);
...
// 	mode: 0, //0省电模式 1流氓模式
// });
// let rsult = hgService.checkIfLimited();
...
59. android.permission
	.READ_EXTERNAL_STORAGE 程序可以读取设备外部存储空间（ 内置SDcard和外置SDCard） 的文件， 如果您的App已经添加了“ WRITE_EXTERNAL_STORAGE” 权限， 则就没必要添加读的权限了， 写权限已经包含了读权限了。
